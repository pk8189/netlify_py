...
def test_upload_200_success_default():
    """Tests a PUT request to the /deploys/{deploy_id}/functions/{name} endpoint.
    ...
    response = client.deploys.functions.upload(
        data=open("./file.txt", "rb"), deploy_id="string", name="string"
    )
    try:
        pydantic.TypeAdapter(models.Function).validate_python(response)
        is_json = True
...
async def test_await_upload_200_success_default():
    ...
    response = await client.deploys.functions.upload(
        data=open("./file.txt", "rb"), deploy_id="string", name="string"
    )
    try:
        pydantic.TypeAdapter(models.Function).validate_python(response)
