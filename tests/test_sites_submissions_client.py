...
def test_list_200_generated_success():
    """Tests a GET request to the /sites/{site_id}/submissions endpoint.
    ...
    response = client.sites.submissions.list(site_id="string")
    try:
...
async def test_await_list_200_generated_success():
    ...
    response = await client.sites.submissions.list(site_id="string")
    try:
        pydantic.TypeAdapter(typing.List[models.Submission]).validate_python(response)
        is_json = True
    except pydantic.ValidationError:
